...
{
    /// <summary>
    /// Minimal turn AI — one decision per turn.
    /// Priority:
    ///   1. Active ability: use one that can kill the enemy Queen; else most damage.
    ///   2. Basic attack: melee/ranged if a target is in effective attack range.
    ///   3. Move toward primary target (enemy Queen if alive, else nearest foe).
    ///   4. Pass.
    /// </summary>
